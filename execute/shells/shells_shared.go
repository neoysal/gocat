...
func runShellExecutor(cmd exec.Cmd, timeout int) ([]byte, string, string) {
	...
	//setCmdLine(cmd.SysProcAttr, cmd.Args) //Windows case to evade escape special chars (i.e double quotes) in the command to execute
...
func runShellExecutorUnquoted(cmd exec.Cmd, timeout int) ([]byte, string, string) {
	done := make(chan error, 1)
	status := execute.SUCCESS_STATUS
	var stdoutBuf, stderrBuf bytes.Buffer
	cmd.SysProcAttr = getPlatformSysProcAttrs()
